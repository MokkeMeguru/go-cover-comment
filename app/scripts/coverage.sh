...
python3 ${script_source}/coverage_stats.py $(git diff HEAD origin/$3 --name-only) --repo $1 --commit_hash $commit_hash
python3 ${script_source}/coverage_details.py $(git diff HEAD origin/$3 --name-only) --repo $1 --commit_hash $commit_hash
